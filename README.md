# 👋 Hi, I'm Harry

Welcome to my GitHub!

I'm a Cloud Engineer, currently on a journey to become a certified and experienced Cloud Architect. I specialise in Microsoft Azure and Cloudflare, and I’m constantly expanding my skills in infrastructure-as-code, automation, and cloud-native technologies.

## 🧠 About Me

- 💼 Working as a Cloud Project Engineer
- 📚 Always looking for new things to learn - I enjoy studying cloud technologies, automation, architecture patterns and lots of homelabbing
  
## 🛠️ Skills & Interests

- ☁️ Platforms: Azure, Linux, Cloudflare, Windows
- ⚙️ IaC: Terraform
- 🖥️ Scripting: PowerShell
- 🔐 Identity & Security: EntraID, AD, IAM, Conditional Access, RBAC
- 📦 Automation & DevOps: GitHub Actions, Azure DevOps

## 📫 Let's Connect

- 🌐 Website: [harryshelton.com](https://harryshelton.com/)
- 💼 LinkedIn: [linkedin.com/in/harryshelton](https://www.linkedin.com/in/harryshelton/)
